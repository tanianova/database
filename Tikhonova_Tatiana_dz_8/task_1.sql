...
-- подсчет количества лайко у каждого пользователя (из 10 самых молодых)
SELECT 
  DISTINCT profiles.user_id, COUNT(target_types.id) AS total
FROM 
  profiles 
LEFT  JOIN
  likes
...
SELECT SUM(likes_total) FROM
  (SELECT COUNT(target_types.id) AS likes_total
FROM 
  profiles 
LEFT  JOIN
  likes
...
SELECT  
  DISTINCT users.id, users.first_name, users.last_name, posts.views_counter 
FROM 
  users 
RIGHT JOIN  
  posts
